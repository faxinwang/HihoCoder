...
对于数字 i= 10011, 将其当做
 ...
 这里用了链接表保存图结构，特别的是，连接表的表头编号表示的是顶点编号，而表头对应的向量里面保存
 的却是与之相连的边的编号而不是顶点编号，并且在深搜的时候，又通过边找到对应的下一个顶点。这一点是我
 之前没有用过的方法。
*/
...
vector<int> g[maxn];
int path[maxn*2], vis[maxn], to[maxn];
int cnt=0;
...
freopen("in.txt","r",stdin);
    ...
    //方法一， 逆向输出
    for(int i=N-2; i>=0; --i) printf("%d", path[cnt] & (1<<i));
    for(int i=cnt-1; i>=N; --i) printf("%d", path[i] & 1);
