...
内存限制:256MB
描述
小Hi和小Ho周末在公园溜达。公园有一堆围成环形的石板，小Hi和小Ho分别站在不同的石板上。
已知石板总共有m块，编号为 0..m-1，小Hi一开始站在s1号石板上，小Ho一开始站在s2号石板上。

小Hi：小Ho，你说我们俩如果从现在开始按照固定的间隔数同时同向移动，我们会不会在某个时
间点站在同一块石板上呢？
...
一个朴素的算法是枚举1~min(A,B)，最大的一个能同时被A,B整除的数即gcd(A,B)。显然
这个算法是非常没有效率的。
...
在判定有解之后，我们需要在其基础上再求出一组(x,y)。由于A,B,C均是gcd(A,B)的整数倍，
因此可以将它们都缩小gcd(A,B)倍。即A'=A/gcd(A,B),B'=B/gcd(A,B),C'=C/gcd(A,B)。

化简为A'x+B'y=C'，gcd(A',B')=1，即A',B'互质。

此时，我们可以先求解出A'x+B'y=1的解(x',y')，再将其扩大C'倍，即为我们要求的最后
解(x,y)=(C'x', C'y')。
