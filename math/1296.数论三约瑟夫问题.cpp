...
首先N个候选人围成一个圈，依次编号为0..N-1。然后随机抽选一个数K，并0号候选人开始
按从1到K的顺序依次报数，N-1号候选人报数之后，又再次从0开始。当有人报到K时，这个人
被淘汰，从圈里出去。下一个人从1开始重新报数。

也就是说每报K个数字，都会淘汰一人。这样经过N-1轮报数之后，圈内就只剩下1个人了，
这个人就作为新的班长。
...
则对应有：
  ...
  0     1 2 3 ... K-2  # K  K+1 K+2 ... n-1
n-K'              n-2'   0'  1'  2' ... n-K-1'
此时在0',1',...,n-2'上再进行一次K报数的选择。而f[n-1]的值已经求得，因此我们可以直接求得
当选者的编号s'。

但是，该编号s'是在n-1个候选人报数时的编号，并不等于n个人时的编号，所以我们还需要将s'转换为
对应的s。
...
至此递推公式的两个式子我们均证明了其正确性，则对于任意给定的n，我们可以使用该递推式求得f[n]，
写成伪代码为：
...
小Hi：那么我们就可以用第二种递推，解决的思路仍然和上面相同，而区别在于我们每次减少的N的规
模不再是1。
...
0 1 2 - 4 5 6 - 8 9
在这里一轮报数当中，有两名候选人退出了。而对于任意一个N,K来说，退出的候选人数量为
N/K("/"运算表示整除，即带余除法取商)
...
若f[8]=2，对于原来的序列来说对应了0，(2+8) mod 10 = 0，是对应的；若f[8]=6，则有
(6+8) mod 10 = 4，然而实际上应该对应的编号为5。
